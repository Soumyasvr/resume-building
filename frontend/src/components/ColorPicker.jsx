...
const ColorPicker = ({selectedColor, onChange}) => {
  ...
  return (
    <div className='relative'>
        ...
        from-purple-50 to-purple-00 ring-purple-300 hover:ring transition-all px-3 py-2 rounded-lg'>
        ...
        {isOpen && (
            ...
            {colors.map((color) =>(
                ...
                onClick={() => {onChange(color.value); setIsOpen(false)}}>
                    ...
                    <p className='text-xs mt-1 text-center text-gray-600'>{color.name}</p> 
                </div>    
            ))}
            </div>    
        )}
