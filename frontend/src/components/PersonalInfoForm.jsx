...
const PersonalInfoForm = ({data, onChange, removeBackground, setRemoveBackground, accentColor}) => {
    ...
    console.log("Accent Color in PersonalInfoForm:", accentColor);
    return (
    <div>
      ...
      <p className="text-gray-600 text-sm">Get Started with the personal information</p>
        <div className='flex items-center gap-2'>
            <label>
                {data.image ? (
                    <img src={typeof data.image ==='string' ? data.image: URL.createObjectURL(data.image)} alt="" className="w-16 h-16 rounded-full object-cover 
                    mt-5 hover:opacity-80" style={{ borderColor: accentColor }} />
                ): (
            ...
            {fields.map((field)=>{
                ...
                return (
                    <div key={field.key} className='space-y-1 mt-5'>
                        ...
                        outline-none transition-colors text-sm mt-1" 
